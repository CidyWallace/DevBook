...
// Criar insere um publicação no banco de dados
...
func (repositorio Publicacoes) Buscar(usuarioID uint64) ([]modelos.Publicacao, error) {
	linhas, erro := repositorio.db.Query(`
		select distinct p.*, u.nick from publicacoes p inner join
		usuarios u on u.id = p.autor_id inner join seguidores s
		on p.autor_id = s.usuario_id where u.id = ? or s.seguidor_id = ?
		order by 1 desc
	`, usuarioID, usuarioID)
	if erro != nil {
		return nil, erro
...
// BuscarPorUsuario trás todas as publicações de um usuário específico
...
// Descurtir remove uma curtida de uma publicação
